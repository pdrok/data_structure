...
// Ivariant: size == number of elements in data
struct array {
    size_t size;   // mumber of elemets in data;
    int *data;     // malloc'd block
};

#define SIZE_INITIAL (16)
#define SIZE_MULTIPLIER (2)   // Minimun ratio to grow by.
...
static void arrayExpand(struct array *a, size_t position)
{
	...
	// Break invariant, we will fix later
...
{
    if(position >= a->size) {
	// No need to expand, just return the default vlaue.
	return 0;
    } else {
...
// Free all space unsed by an array.
